...
// Google service-account keys break in a dozen small ways when pasted into a
// hosting dashboard. This normalizes the common mistakes that cause
// "1E08010C:DECODER routines::unsupported":
//   - surrounding single/double quotes that the dashboard kept literally
...
function normalizePrivateKey(raw) {
  ...
  // Turn literal \n (and \r) escapes into real newlines, drop stray \r.
  k = k.replace(/\\r/g, '').replace(/\\n/g, '\n').replace(/\r/g, '');
  return k.trim() + '\n'; // PEM should end with a trailing newline
}
...
async function getSheet() {
  ...
  // Surface obvious misconfig early with a clear message (without leaking the key).
  if (!email) console.warn('⚠️  GOOGLE_SERVICE_ACCOUNT_EMAIL not set');
  if (!key || !key.includes('BEGIN PRIVATE KEY')) {
    console.error('❌ GOOGLE_PRIVATE_KEY looks malformed (missing "BEGIN PRIVATE KEY"). ' +
      'Paste the full key from the service-account JSON, including the BEGIN/END lines.');
  }
